...
- **Cards embaralhados no topo**, abaixo da subheadline (referência: template "Ai-Boost"): 5 cards inclinados com imagem, categoria e selo verde, e "+500 wallpapers premium" ao lado. Sem números de visualizações/curtidas (seriam inventados). Mídias reais desde 2026-09-15, em `site/assets/cards/` (recorte central 3:4, 360×480): Anime = vídeo do Gojo (Jujutsu Kaisen, usado a pedido após alerta de direitos autorais), Old Money = "Money talks", Cyberpunk (meio) = rosto com viseira azul, Minimalista = "Amour", Lo-fi = nota de 100 amassada. Para trocar, mudar `src` em `HERO_CARDS` no `main.js`.
- Botões de compra renomeados para **"Liberar 500+"** (EN "Unlock 500+"). Para não dar a entender que é grátis, o preço aparece logo abaixo: "Acesso completo por R$ 5,00 · pagamento único" (`PRICE` no `main.js`, precisa bater com a Kiwify; lido do checkout em 2026-09-15).
- **Avaliações** (avatares + 4 de 5 estrelas) agora aparecem para todos, a pedido, porque o site é uma amostra. Sempre com a etiqueta **"Avaliações ilustrativas"**. Com avaliações reais, trocar a nota e tirar a etiqueta. Contador e notificação de compra seguem só no `?demo=1`.
- **Inconsistência a resolver:** a seção de preços ainda mostra os planos Grátis/Pro (R$ 14,90/mês), mas o pack custa R$ 5,00 em pagamento único.

...
- **Cards embaralhados no topo**, abaixo da subheadline (referência: template "Ai-Boost"): 5 cards inclinados com imagem, categoria e selo verde, e "+500 wallpapers premium" ao lado. Sem números de visualizações/curtidas (seriam inventados). Imagens provisórias; para trocar, preencher `src` em `HERO_CARDS` no `main.js` e salvar os arquivos em `site/assets/cards/`.
